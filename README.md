...
<p align="center">
  <a href="https://github.com/Vitaliy888/vless-shop-tgbot?tab=readme-ov-file#%EF%B8%8F-%D1%83%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%BA%D0%B0-%D0%BF%D0%BE%D0%B4-%D0%BA%D0%BB%D1%8E%D1%87">Установка и обновление</a> •
  <a href="https://github.com/Vitaliy888/vless-shop-tgbot?tab=readme-ov-file#-%D0%B1%D0%B0%D0%B3%D0%B8-%D0%B8-%D0%BF%D1%80%D0%B5%D0%B4%D0%BB%D0%BE%D0%B6%D0%B5%D0%BD%D0%B8%D1%8F">Баги и предложения</a> •
  <a href="https://github.com/Vitaliy888/vless-shop-tgbot?tab=readme-ov-file#-%D0%BF%D0%BE%D0%B4%D0%B4%D0%B5%D1%80%D0%B6%D0%B0%D1%82%D1%8C-%D1%80%D0%B0%D0%B7%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%BA%D1%83">Поддержать проект</a>
</p>

![GitHub License](https://img.shields.io/github/license/Vitaliy888/vless-shop-tgbot)
![GitHub Release](https://img.shields.io/github/v/release/Vitaliy888/vless-shop-tgbot)
![GitHub Issues or Pull Requests](https://img.shields.io/github/issues/Vitaliy888/vless-shop-tgbot)
![GitHub Stars](https://img.shields.io/github/stars/Vitaliy888/vless-shop-tgbot?style=flat)
...
- **Веб-панель**: Раздельные разделы: `Серверы` (управление хостами и тарифами), `Настройки` (логин/пароль панели, токены, платежи, контент, пробный период, рефералы), `Пользователи`, `Dashboard`.
...
```bash
curl -sSL https://raw.githubusercontent.com/Vitaliy888/vless-shop-tgbot/main/install.sh | sudo bash
```

**Если вы обновляете бота повторно, инструкции ниже не появятся!**
...
После установки выполните эти шаги, чтобы бот заработал:

1. **Войдите в веб-панель** по адресу `https://your-domain.com/login`.
2. **Сразу смените логин и пароль**: Страница `Настройки` → блок "Настройки Панели".
3. **Заполните обязательные поля Telegram**: `Токен бота`, `Username бота (без @)`, `Telegram ID Администратора`.
4. **Добавьте сервер**: Перейдите в раздел `Серверы` → заполните URL панели 3x-ui, порт, логин и пароль → Добавить.
5. **Создайте тарифы**: На странице `Серверы` в блоке выбранного хоста создайте тариф (например, 30 дней / 100 RUB).
6. (Опционально) Включите `Пробный период` или `Реферальную систему` в разделе `Настройки` → соответствующие блоки.
7. **Сохраните настройки**: Кнопка внизу страницы `Настройки`.
8. **Запустите бота**: В шапке панели нажмите зелёную кнопку "Запустить бота".

Готово — бот начнёт обслуживать пользователей.

### Реферальная система
Включается переключателем "Включить реферальную систему". При отключении:
- Кнопка рефералов исчезает из меню бота.
- Переход по реферальным ссылкам игнорируется.
- Начисление бонусов и скидка новым пользователям не применяются.

Параметры:
- `Процент вознаграждения` — сколько получает пригласивший.
- `Скидка для нового пользователя` — снижает цену первой покупки.
- `Минимальная сумма для вывода` — порог для запроса выплаты.
...
## 🔄 Обновление структуры (Servers vs Settings)

Если вы обновились с версии, где управление хостами было внутри `Настройки`, теперь оно находится в отдельном разделе `Серверы`. Никаких дополнительных действий не требуется — данные сохранены в базе.

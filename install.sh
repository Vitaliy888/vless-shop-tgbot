...
if [ -d "$INSTALL_DIR" ]; then
    echo -e "\n${CYAN}Обнаружена существующая папка установки. Запускаем режим обновления.${NC}"

    cd "$INSTALL_DIR"

    # show installed version if exists
    if [ -f "VERSION" ]; then
        echo -e "Current installed version: $(cat VERSION)"
    fi
    ...
    echo -e "${GREEN}✔ Код успешно обновлен (если были изменения).${NC}"
...
# checkout requested ref if provided
if [ -n "$GIT_REF" ]; then
    git checkout "$GIT_REF" || true
fi

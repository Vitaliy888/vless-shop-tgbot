...
def run_migration():
    ...
    # migrate xui_hosts to add max_users column if missing
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        ...
        if 'max_users' not in cols:
            cursor.execute("ALTER TABLE xui_hosts ADD COLUMN max_users INTEGER")
            logging.info(" -> The column 'max_users' is successfully added to xui_hosts.")
        else:
            logging.info(" -> The column 'max_users' already exists in xui_hosts.")
        conn.commit()
        conn.close()
    except sqlite3.Error as e:
        logging.error(f"An error occurred during xui_hosts migration: {e}")
...
def create_host(name: str, url: str, user: str, passwd: str, inbound: int):
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO xui_hosts (host_name, host_url, host_username, host_pass, host_inbound_id, max_users) VALUES (?, ?, ?, ?, ?, ?)",
                (name, url, user, passwd, inbound, None)
            )
            conn.commit()
...
def update_key_name(key_id: int, new_name: str) -> bool:
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE vpn_keys SET key_name = ? WHERE key_id = ?", (new_name, key_id))
            conn.commit()
            return True
    except sqlite3.Error as e:
        logging.error(f"Failed to update key name for {key_id}: {e}")
        return False

def update_key_max_connections(key_id: int, max_connections: int | None) -> bool:
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            if max_connections is None:
                cursor.execute("UPDATE vpn_keys SET max_connections = NULL WHERE key_id = ?", (key_id,))
            else:
                cursor.execute("UPDATE vpn_keys SET max_connections = ? WHERE key_id = ?", (int(max_connections), key_id))
            conn.commit()
            return True
    except sqlite3.Error as e:
        logging.error(f"Failed to update max_connections for {key_id}: {e}")
        return False
...
def get_current_users_count_for_host(host_name: str) -> int:
    """Return number of unique user_ids who have keys on the given host."""
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(DISTINCT user_id) FROM vpn_keys WHERE host_name = ?", (host_name,))
            result = cursor.fetchone()
            return int(result[0]) if result and result[0] is not None else 0
    except sqlite3.Error as e:
        logging.error(f"Failed to get current users count for host '{host_name}': {e}")
        return 0
...
def delete_user(user_id: int):
    """Delete user and related data: vpn_keys, transactions, support thread, and user record."""
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            # delete vpn keys
            ...
            # delete support thread entry if exists
            cursor.execute("DELETE FROM support_threads WHERE user_id = ?", (user_id,))
            # delete user record
            cursor.execute("DELETE FROM users WHERE telegram_id = ?", (user_id,))
            conn.commit()
            logging.info(f"Deleted user {user_id} and related data.")
            return True
    except sqlite3.Error as e:

...
def get_user_router() -> Router:
    ...
    async def howto_android_handler(callback: types.CallbackQuery):
        ...
        await callback.message.edit_text(
            ...
            "• <a href=\"https://play.google.com/store/search?q=clash%20vpn&c=apps\">Clash / Clash Meta</a> (может отсутствовать)\n"
            "• <a href=\"https://play.google.com/store/search?q=nekobox&c=apps\">NekoBox</a> (может отсутствовать)\n\n"
    ...
    async def howto_ios_handler(callback: types.CallbackQuery):
        ...
        await callback.message.edit_text(
            ...
            "• <a href=\"https://apps.apple.com/search?term=Streisand\">Streisand</a>\n"
            "• <a href=\"https://apps.apple.com/search?term=FoXray\">FoXray</a>\n"
            "• <a href=\"https://apps.apple.com/app/v2box-v2ray-client/id6446814690\">V2Box - V2ray Client</a>\n"
            "• <a href=\"https://apps.apple.com/search?term=v2raytun\">v2RayTun</a>\n"
            "• Hiddify (через TestFlight — может быть временно недоступно)\n\n"
            "<b>Шаги (универсально):</b>\n"
...
async def process_successful_payment(bot: Bot, metadata: dict):
    ...
    try:
        ...
        if action == "new":
            ...
            if host_info and host_info.get('max_users'):
                try:
                    current_users = get_current_users_count_for_host(host_name)
                    if current_users >= int(host_info.get('max_users')):
                        await processing_message.edit_text(f"❌ Достигнут лимит пользователей на сервере '{host_name}'. Обратитесь к администратору.")
                        return
                except Exception:
                    # if check fails, continue and try to create key to avoid false blocks

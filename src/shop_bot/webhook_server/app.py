...
def create_webhook_app(bot_controller_instance):
    ...
    @flask_app.route('/edit-key/<int:key_id>', methods=['POST'])
    @login_required
    def edit_key_route(key_id):
        new_name = request.form.get('key_name', '').strip()
        max_conn_raw = request.form.get('max_connections', '').strip()
        max_conn = None
        if max_conn_raw:
            try:
                ...
                flash('Параметр "Макс подключений" должен быть положительным числом.', 'danger')
        ...
        name_ok = True
        if new_name:
            name_ok = update_key_name(key_id, new_name)

        conn_ok = update_key_max_connections(key_id, max_conn)

        # try to apply on panel
        try:
            host = get_host(key['host_name'])
            if host:
                api, inbound = xui_api.login_to_host(host['host_url'], host['host_username'], host['host_pass'], host['host_inbound_id'])
                if api and inbound:
                    xui_api.set_client_max_connections(api, inbound.id, key['xui_client_uuid'], max_conn)
        except Exception:
            logger.exception('Failed to apply max_connections on panel; value saved in DB.')

        if name_ok and conn_ok:
            flash('Данные ключа обновлены.', 'success')
        else:
            flash('Не удалось обновить некоторые параметры ключа. Проверьте логи.', 'warning')
    ...
    def settings_page():
        if request.method == 'POST':
            ...
            # enforce_max_connections is a new checkbox setting
            values = request.form.getlist('enforce_max_connections')
            value = values[-1] if values else 'false'
            update_setting('enforce_max_connections', 'true' if value == 'true' else 'false')
    ...
    def update_code_route():
        # best-effort perform git pull in repository root
        repo_root = Path(__file__).resolve().parents[2]
        try:
            proc = subprocess.run(['git', 'pull'], cwd=str(repo_root), capture_output=True, text=True, timeout=120)
            out = proc.stdout + '\n' + proc.stderr
            logger.info(f"Git pull output: {out}")
            flash('Обновление кода выполнено. Проверьте логи для подробностей.', 'success')
            # try to restart docker-compose if present
            ...
            if dc_file.exists():
                ...
                flash('Docker-контейнеры пересобраны и перезапущены.', 'success')
        except Exception as e:
            logger.exception('Failed to update code')
            flash('Не удалось выполнить обновление. Проверьте логи.', 'danger')
    ...
    def delete_user_route(user_id):
        ...
        if success:
            flash(f'Пользователь {user_id} и связанные данные были удалены.', 'success')
        else:
            flash('Не удалось удалить пользователя. Проверьте логи.', 'danger')
        return redirect(url_for('users_page'))
        ...
        if max_users is not None:
            try:
                ...
                with _sqlite.connect(DB_FILE) as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE xui_hosts SET max_users = ? WHERE host_name = ?", (max_users, request.form['host_name']))
                    conn.commit()
            except Exception:

...
def set_client_max_connections(api: Api, inbound_id: int, client_uuid: str, max_connections: int | None) -> bool:
    """Best-effort: set a property on a client that limits simultaneous connections.
    The exact property name depends on the panel implementation. This function will try common field names
    and fall back to enabling/disabling the client if max_connections == 0.
    """
    try:
        inbound = api.inbound.get_by_id(inbound_id)
        if not inbound:
            logger.error(f"Inbound {inbound_id} not found when setting max connections")
            return False

        updated = False
        for client in (inbound.settings.clients or []):
            if getattr(client, 'id', None) == client_uuid:
                # try common attribute names
                if max_connections is None:
                    # remove limit if possible
                    for attr in ('max_connections', 'concurrency', 'connection_limit'):
                    ...
                    for attr in ('max_connections', 'concurrency', 'connection_limit'):
                        try:
                            setattr(client, attr, int(max_connections))
                            updated = True
                            break
                        except Exception:
                            continue
        ...
        if updated:
            ...
            logger.info(f"Applied max_connections={max_connections} for client {client_uuid} on inbound {inbound_id}")
            return True
        else:
            logger.info(f"Could not apply max_connections property for client {client_uuid} - property not found; will rely on scheduler enforcement")
            return False
    except Exception as e:
        logger.error(f"Error while setting max connections on panel: {e}", exc_info=True)
        return False